public class Program {
    ...
    public static Winner determineWinner(HandShape p1, HandShape p2) {
        if(p1 == HandShape.ROCK) {
            if(p2 == HandShape.ROCK) return Winner.DRAW;
            else if(p2 == HandShape.PAPER) return Winner.PLAYER_TWO;
            else return Winner.PLAYER_ONE;
        }
        else if(p1 == HandShape.SCISSORS) {
            if(p2 == HandShape.SCISSORS) return Winner.DRAW;
            else if(p2 == HandShape.ROCK) return Winner.PLAYER_TWO;
            else return Winner.PLAYER_ONE;
        }
        else {
            if(p2 == HandShape.PAPER) return Winner.DRAW;
            else if(p2 == HandShape.SCISSORS) return Winner.PLAYER_TWO;
            else return Winner.PLAYER_ONE;
        }
    }
